...
**한 줄 소개**: 아이디어를 즉시 공유하고, 실시간으로 함께 발전시키는 미니멀 온라인 화이트보드
**개발 기간**: 2025년 12월
...
- 혼자 적는 보드가 아니라 함께 만드는 협업 보드
- "열면 터진다 / 공유하면 더 커진다" 느낌의 에너지 & 스파클 컨셉
...
- **키워드**: Warm · Bold · Energy · Classic
- **메인 컬러**: 진붉은 레드 (#E10600 계열)
- **포인트**: 버블 / 탄산 파티클 / 부드러운 곡선

**🌙 다크 모드 = 제로 콜라 + 라임 포인트**

- **키워드**: Modern · Sharp · Neon · Tech
- **배경**: 제로콜라 블랙
- **포인트 컬러**: 라임 네온 (#9EF01A 계열)
- **느낌**: "검은 배경 속에서 빛나는 협업 시그널"
...
#### 6.3 반응형 디자인
- **모바일 지원**: 완전한 모바일 반응형 레이아웃 구현
- **터치 제스처**: 
  - 캔버스 패닝: 터치 드래그로 캔버스 이동
  - 요소 선택: 터치로 요소 선택 가능
  - 더블 탭 편집: 요소 더블 탭으로 편집 모드 진입
- **모바일 UI 최적화**:
  - 보드 툴바: 세로 레이아웃, 아이콘만 표시
  - 협업 위젯: 고정 위치 (왼쪽 상단)
  - 미니맵: 우측 상단 고정 위치
  - 줌 컨트롤: 하단 고정 위치 조정
- **모바일 배너**: 모바일 환경 제한 사항 안내 및 "다시 보지 않기" 옵션
- **랜딩 페이지**: 메인 타이틀 및 화이트보드 데모 섹션 모바일 최적화
...
- **모바일 환경**: 
  - 요소 드래그/리사이즈는 PC에서만 지원 (모바일에서는 선택 및 편집만 가능)
  - 일부 고급 기능은 PC 사용 권장
